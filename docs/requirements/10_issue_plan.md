...
|---|---|---|---|---|---|---|---:|---|---|
| I-01 建立最小工程骨架 | 让后续功能有统一入口；当前仓库仅有数据/文档 | 创建包、tests、配置/输出目录、依赖和 CLI help | `src/`, `tests/`, `config/`, README | 指定环境可运行 `--help`，无训练/UI | import、CLI smoke、pytest | high | 2 | 阶段一验收 | `type:feature`, `priority:high`, `stage:2` |
...
| I-03 路径保护和日志 | 防止覆盖原始数据并保留证据 | 根路径发现、输出创建、原始路径保护、日志初始化 | `paths.py`, `logging_setup.py` | 输出不指向原 CSV，日志不重复 | 路径边界、重复初始化测试 | high | 2 | I-02 | `type:feature`, `type:test`, `stage:2` |
...
| I-10 统一评估与调优 | 满足指标并避免测试选参 | 指标 schema、矩阵/报告、训练验证调参、锁定后测试 | `evaluation/`, `experiments.py` | 两模型基线/优化/最终结果可追溯 | 指标对照、测试集隔离测试 | high | 3 | I-08、I-09 | `type:model`, `type:test`, `stage:3` |
...
| I-14 选择 UI 框架与应用服务 | 降低界面对核心逻辑耦合 | M5/Python3.10 最小验证，定义服务/ViewModel | `app/services.py`, ADR | 框架依据和最小 smoke 证据齐全 | 启动、服务 mock 测试 | high | 5 | 阶段四通过 | `type:ui`, `priority:high`, `stage:5` |
...
## 测试方法
`conda run -p /Users/liang/dev/envs/workspace pytest ...`
