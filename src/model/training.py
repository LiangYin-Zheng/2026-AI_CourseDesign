...
def train_sklearn_model(
    ...
    best_candidate_key: tuple[float, float, float, int] | None = None
    for candidate_index, raw_candidate in enumerate(candidates):
        parameters = normalize_candidate(model_name, raw_candidate)
        pipeline = Pipeline(
            [
                ("preprocessor", clone(prepared.preprocessor)),
                (
                    "classifier",
                    build_sklearn_classifier(model_name, parameters, config),
                ),
            ]
        )
        started = perf_counter()
        # Apple Accelerate 可能让矩阵乘法继承异常浮点状态；
        # 这里通过概率有限性做实质校验。
