...
class Predictor:
    def __init__(self, bundle: dict) -> None:
        ...
        if set(self.numeric_columns + self.categorical_columns) != set(self.input_columns):
    ...
    def _validate_frame(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in self.numeric_columns:
            ...
            if not isinstance(bounds, dict) or not {"minimum", "maximum"} <= set(bounds):
    ...
    def _predict_probabilities(self, frame: pd.DataFrame) -> np.ndarray:
        ...
        if self.bundle["mode"] == "manual":
            ...
            return np.asarray(self.bundle["model"].predict_proba(transformed), dtype=np.float64)
            ...
            predictions.append(
                {
                    ...
                    "probabilities": {
                        ...
                        for class_name, probability in zip(class_names, probabilities[row_index])
                    },
