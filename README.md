...
- **项目级日志体系**：日志写入 `output/logs/project.log`，运行时优先使用 `loguru`，未安装时自动回退到标准库 `logging`。
- **结构化源码组织**：主实现已按模块归档。
...
1. **运行时终端日志**：重点展示整体进度与关键状态。
2. **文件日志**：记录更完整的项目运行信息，便于回溯和排错。

当前实现优先使用 `loguru` 管理日志 sink 和等级；如果当前环境尚未安装 `loguru`，程序会自动回退到标准库 `logging`，不会影响基本运行。
